...
export default function FormVenta({ onGuardado, onCerrar }) {
  ...
  const actualizarLinea = async (id, campo, valor) => {
    ...
    if ((campo === 'producto_id' || campo === 'tipo_venta') && producto_id) {
      try {
        ...
        setLineas((prev) =>
          prev.map((l) =>
            l.id === id ? { ...l, [campo]: valor, precio_unitario: precio ? Number(precio.valor) : '' } : l
          )
        )
      } catch {
  ...
  const totalGeneral = lineas.reduce((sum, l) => {
    const t = l.cantidad_kg && l.precio_unitario
                ...
                <div className="flex rounded-lg overflow-hidden border border-gray-300">
                  {['detal', 'mayor'].map((t) => (
                    <button
                      key={t}
                      type="button"
                      onClick={() => actualizarLinea(linea.id, 'tipo_venta', t === 'mayor' ? 'mayorista' : 'detal')}
                      className={`flex-1 py-2 text-xs font-medium transition-colors ${
                        (t === 'mayor' ? 'mayorista' : 'detal') === linea.tipo_venta
